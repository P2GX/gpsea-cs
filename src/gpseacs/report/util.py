...
def get_fet_result_table(fet_d) -> typing.List[str]:
    ...
    for res in sig_result_list:
        ...
        rows.append(row)
    latex_rows = list()
    latex_rows.append("\\begin{subfigure}[b]{0.9\\textwidth}")
    latex_rows.append("\\centering")
    latex_rows.append("\\begin{tabular}{lllrr}")
...
def get_mono_result_table(mono_d) -> typing.List[str]:
    ...
    desc = mono_d.get("description")
    pval = mono_d.get("pval")
    latex_caption = f"{test_name} to compare {a_geno} and {b_geno} with respect to {var_name}."
    header = ["Description", "Variable", "Genotype (A)", "Genotype (B)", "p-value"]
    row = [desc, var_name, a_geno, b_geno, format_for_latex(pval)]
    latex_rows = list()
    latex_rows.append("\\begin{subfigure}[b]{0.9\\textwidth}")
    latex_rows.append("\\centering")
    latex_rows.append("\\begin{tabular}{lllrr}")
...
def process_latex(context_d: typing.Dict, 
    ...
    output_tex_file = f"../../supplement/tex/{cohort}_summary.tex"
